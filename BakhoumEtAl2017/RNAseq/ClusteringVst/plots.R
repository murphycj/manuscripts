...
perform_pcas <- function(data,sample_data) {
	...
	for (i in 1:NUMBER_PCS) {
		for (j in 1: NUMBER_PCS) {
			...
			legend(x="topright",names(cohort[[2]]),cex=0.85,inset=c(-0.35,0),pch=16,col=legend_colors,title="Group")
			dev.off()
		}
	}
}
...
NUMBER_PCS = 4
data <- read.csv("../HTSeqCount/HTSeq.gene.counts.vst.csv",row.names=1,check.names=F)
data[data<=0.1]=0.0
...
spearman <- function(x, ...) {
    x <- as.matrix(x)
    res <- as.dist(1 - cor(x, method = "spearman", use = "complete.obs"))
    res <- as.dist(res)
...
result <- parPvclust(cl, data, method.dist=spearman, method.hclust="average", nboot=1000)
png("./Cluster/pvclust-allGenes.png",width=1200,height=600)
par(mar=c(11,7,7,7))
plot(result)
dev.off()
