...
data <- read.csv("../../HTSeqCount/HTSeq.geneSymbol.human.counts.csv",row.names=1, check.names=FALSE,header=T)
data <- data[rowSums(data)>1,]
data <- data[,c("A1","A2","A5","A6","NAIVE","LIPO","EN15d","EN4d","EN_M","EN_F","NAIVE_F","A1_plus","A2_plus","A5_plus","A6_plus")]
coldata <- data.frame(
	treatment=c(
		rep("APC-WT",length(c("A1","A2","A5","A6","NAIVE","LIPO","EN15d","EN4d","EN_M","EN_F","NAIVE_F"))),
		rep("APC-MUT",length(c("A1_plus","A2_plus","A5_plus","A6_plus")))
	),
...
res <- results(result)
plot_and_save_results(res, "APCWT_vs_APCMUT")
